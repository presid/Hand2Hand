  ...
  $vpath="";
         ...
         if (isset($_POST['Insert_post'])){
                // getting the text data from the insertion fields 
                 ...
                 $inserting_post ="insert into posts(post_title,post_desc,post_image,path)
			...
			if(isset($_POST["button_edit"])){
				   ...
				   if($post_image=="")
				   {
				$qry=mysqli_query($con,"update posts set post_title='".$_POST["post_title"]."',path='".$_POST["path"]."',post_desc='".$_POST["post_desc"]."' where post_id='".$_POST["post_id"]."'");
			}else{
				 
                 $post_image_tmp=$_FILES['post_image']['tmp_name'];
                 move_uploaded_file($post_image_tmp,"product_images/$post_image");
				 $qry=mysqli_query($con,"update posts set post_image='$post_image', post_title='".$_POST["post_title"]."',path='".$_POST["path"]."',post_desc='".$_POST["post_desc"]."' where post_id='".$_POST["post_id"]."'");
			}
			}
			  if(isset($_GET["delete"])){
				$qry = mysqli_query($con,"delete from posts where post_id='".$_GET["delete"]."'");
				if($qry){
	 			    unlink("product_images/".$_GET["product_images"]);
				}
			  }
			 else if(isset($_GET["edit"])){
				$qry =mysqli_query($con,"select * from posts where post_id='".$_GET["edit"]."'");
...
?>
          <div class="grid_10">
            <div class="box round first">
                <h2>
                    Inserting into Charter Service Second!!!</h2>
                <div class="block">
<form action="post_thirds.php" method ="post" enctype="multipart/form-data">
